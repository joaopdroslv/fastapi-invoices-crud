...
def create_invoice(
    provided_invoice: BasicInvoiceRequest, db: Session = Depends(get_db)
) -> InvoiceResponse:
...
def pay_invoice(
    invoice_id: int, provided_invoice: PaymentInfoRequest, db: Session = Depends(get_db)
) -> InvoiceResponse:
...
def update_invoice(
    invoice_id: int,
    provided_invoice: BasicInvoiceRequest,
