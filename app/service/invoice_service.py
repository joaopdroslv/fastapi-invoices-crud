...
def set_invoice_as_paid(invoice_id: int, payment_info: PaymentInfoRequest, db: Session) -> Invoice:
    ...
    if db_invoice.remaining_value < 0:
        ...
        raise HTTPException(status_code=422, detail='You cannot pay more than the invoice total.')
...
def link_invoice_to_an_user(db_invoice: Invoice, provided_invoice: BasicInvoiceRequest, db: Session):
